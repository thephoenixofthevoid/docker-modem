...
const http_duplex_1 = require("./http_duplex");
var querystring = require('querystring'), http = require('./http'), fs = require('fs'), path = require('path'), url = require('url'), stream = require('readable-stream'), debug = require('debug')('modem'), util = require('util'), splitca = require('split-ca'), JSONStream = require('JSONStream'), isWin = require('os').type() === 'Windows_NT';
...
Modem.prototype.buildPayload = function (err, isStream, statusCodes, openStdin, req, res, json, cb) {
    ...
    if (statusCodes[res.statusCode] !== true) {
        getCause(isStream, res, json, function (err, cause) {
            var msg = new Error('(HTTP code ' + res.statusCode + ') ' +
        ...
        if (openStdin) {
            cb(null, new http_duplex_1.Http_Duplex(req, res));
        }
        else if (isStream) {
            cb(null, res);
        }
...
Modem.prototype.followProgress = function (stream, onFinished, onProgress) {
    var parser = JSONStream.parse(), output = [];
